...
export class Source extends BaseSource<Params> {
  async gather({ denops }: GatherArguments<Params>): Promise<DdcGatherItems> {
    // We have to go through Vim script to guarantee that Denippet is loaded.
    return await denops.call("denippet#get_complete_items", false) as Item<UserData>[];
  }
  ...
  override async getPreviewer(
    { denops, item }: GetPreviewerArguments<Params, UserData>,
  ): Promise<Previewer> {
  ...
  private async snippetToString(denops: Denops, body: string): Promise<string> {
    return await denops.dispatch("denippet", "snippetToString", body) as string;
  }
